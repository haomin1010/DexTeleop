#!/usr/bin/env bash
# Real robot: LEFT arm only.
# Usage: ./scripts/run_tianji_real_teleop_left.sh
set -euo pipefail

ROOT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
CONTAINER_DEXPROJ="${DEXPROJ_CONTAINER_WORKDIR:-/workspace/DexProj}"
CONTROLLER_CONFIG="${TIANJI_REAL_LEFT_CONFIG:-$CONTAINER_DEXPROJ/wuji-hand-teleop/src/output_devices/tianji_output/tianji_output/config/tianji_output_real_teleop_left.yaml}"
ENABLE_MUJOCO="${ENABLE_MUJOCO:-false}"

MUJOCO_ARGS=""
if [ "$ENABLE_MUJOCO" = "true" ] || [ "$ENABLE_MUJOCO" = "1" ]; then
  MUJOCO_ARGS="enable_mujoco:=true mujoco_side:=left"
fi
