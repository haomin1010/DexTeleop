...
colcon build --packages-select controller tianji_output wuji_teleop_bringup --symlink-install 2>&1 | tail -20
set +u
...
echo ''
echo '  Wait for \"Tracker zero initialized\", then ~3s hold + ~5–8s move each (pitch/yaw/roll small then large):'
