...
class TianjiArmControllerNode(Node):
    ...
    def _teleop_control(self):
        ...
        # Query forearm tracker first. Wrist deltas are computed relative to it
        # so elbow motion does not masquerade as wrist pitch/yaw.
        ...
        if left_tf is not None:
            ...
            self.left_pose = self._resolve_tracker_pose("left", left_tf, orientation_tf=left_ori_tf)

        right_tf = self._lookup_transform("right_chest", "tianji_right")
        if right_tf is not None:
            right_ori_tf = self._lookup_orientation_source_tf("right", "right_chest", "tianji_right")
            self.right_pose = self._resolve_tracker_pose("right", right_tf, orientation_tf=right_ori_tf)
        ...
        if self.left_pose is not None or self.right_pose is not None:
            ...
            if self._use_pinocchio_ik and self._tracker_orientation_mode in ("full_pose", "position_only"):
                ...
                if l_joints is not None or r_joints is not None:
                    self.controller.move_to_joints_direct(left_joints=l_joints, right_joints=r_joints)
                    self._publish_command(l_joints, r_joints)
            elif self._tracker_mode == "incremental":
                _, _, l_joints, r_joints = self.controller.move_to_matrix_direct(
                    left_mat=self.left_pose_mat,
                    right_mat=self.right_pose_mat,
    ...
    def _solve_pinocchio_side(self, side: str, target_mat_m: Optional[np.ndarray]) -> Optional[list]:
        ...
        try:
            result = solver.solve(target_for_pin, seed, side=side)
        except Exception as exc:
    ...
    def _resolve_tracker_pose(
        ...
        return self._kine_matrix_to_pose(side, target)
    ...
    def _kine_matrix_to_pose(self, side: str, matrix_m: np.ndarray) -> np.ndarray:
        ...
        if side == "left":
            pose = self.controller.kine_left.mat4x4_to_xyzabc(matrix_list)
        else:
            pose = self.controller.kine_right.mat4x4_to_xyzabc(matrix_list)
        if pose is False:
        ...
        try:
            ...
            if side == "left":
                if left_joints is None:
                    return None
                fk = self.controller.kine_left.fk(0, left_joints)
            else:
                if right_joints is None:
                    return None
                fk = self.controller.kine_right.fk(1, right_joints)
