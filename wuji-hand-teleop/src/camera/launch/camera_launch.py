...
def launch_cameras(context):
    ...
    for cam_key in ['head', 'left_wrist', 'right_wrist']:
        ...
        # USB head camera is handled by unified_stereo below (outputs both ROS2 + PICO H.264)
        # Not started separately in the loop to avoid contention with unified_stereo for the same device
        if cam_key == 'head' and str(cam_config.get('type', 'usb')).lower() == 'usb':
    ...
    # ==================== Head Stereo Camera ====================
    head_cam_type = str(cameras_config.get('head', {}).get('type', 'usb')).lower() if isinstance(cameras_config.get('head', {}), dict) else 'usb'
    if head_cam_type == 'usb' and (enable_head or enable_pico):
