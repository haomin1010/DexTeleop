"""MuJoCo viewer for the Tianji M6 right arm."""
...
class TianjiMujocoViewer(Node):
    ...
    def __init__(
        ...
        super().__init__("tianji_mujoco_viewer")
        if side != "right":
            raise ValueError("Only the right Tianji arm URDF is packaged for now")

        self.side = side
        self.joint_topic = joint_topic
        self.initial_joint_topic = initial_joint_topic
        self.joint_names = [f"Joint{i}_R" for i in range(1, 8)]
    ...
    def _load_model(self) -> mujoco.MjModel:
        ...
        urdf_path = share_dir / "urdf" / "right.urdf"
...
def _parse_args(argv: Optional[list[str]] = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Show Tianji right arm joint states in MuJoCo.")
    parser.add_argument("--side", default="right", choices=["right"])
    parser.add_argument("--joint-topic", default="/tianji_arm/right/joint_command")
    parser.add_argument("--initial-joint-topic", default="/tianji_arm/right/joint_state")
