...
Usage:
    # Right side with Wuji Glove + Tracker
    ros2 launch wuji_teleop_bringup wuji_teleop_single.launch.py side:=right hand_input:=wuji_glove arm_input:=tracker

    # Left side with Wuji Glove + Tracker
    ros2 launch wuji_teleop_bringup wuji_teleop_single.launch.py side:=left hand_input:=wuji_glove arm_input:=tracker

    # With RViz visualization
    ros2 launch wuji_teleop_bringup wuji_teleop_single.launch.py side:=right enable_rviz:=true

Note: Make sure your config files are configured for the correct side.
"""
...
def generate_launch_description() -> LaunchDescription:
    ...
    return LaunchDescription([
        ...
        Node(
            ...
            output="screen",
            emulate_tty=True,
            condition=IfCondition(PythonExpression(["'", hand_input, "' == 'manus' and '", side, "' == 'left'"])),
        ),
        ...
        Node(
            ...
            output="screen",
            emulate_tty=True,
            condition=IfCondition(PythonExpression(["'", hand_input, "' == 'manus' and '", side, "' == 'right'"])),
        ),
        ...
        OpaqueFunction(function=_create_single_side_tf, condition=IfCondition(enable_arm)),
