...
class WujiGloveDevice(InputDeviceBase):
    ...
    def __init__(
        ...
        self._device = self._connect_with_retry(manager, sn, device_name)
        self._sub = self._device.hand_skeleton().subscribe()

    @staticmethod
    def _connect_with_retry(manager, sn: Optional[str], device_name: str, attempts: int = 3):
        last_error = None
        for attempt in range(1, attempts + 1):
            try:
                if sn:
                    return manager.connect(sn=sn, device_name=device_name)
                return manager.auto_connect(device_name=device_name)
            except Exception as exc:
                last_error = exc
                if attempt == attempts:
                    break
                print(
                    f"Wuji glove connect failed for {sn or '<auto>'} "
    ...
    def drain(self) -> None:
        """Drop queued skeleton frames so the next read starts fresh."""
        while self._sub.recv() is not None:
            pass
