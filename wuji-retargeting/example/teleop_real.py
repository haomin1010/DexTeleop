...
class RosJointStatePublisher:
    def __init__(self, topic: str, hand_side: str):
        self.topic = topic
        ...
        try:
            ...
            raise ImportError("--ros-joint-state-topic requires rclpy and ROS message packages.") from exc
        self.rclpy = rclpy
        self.JointState = JointState
        self.Header = Header
        if not rclpy.ok():
            rclpy.init(args=None)
        safe_topic = topic.strip("/").replace("/", "_") or hand_side
        self.node = rclpy.create_node(f"teleop_real_{safe_topic}_publisher")
    ...
    def publish(self, qpos: np.ndarray) -> None:
        if self.publisher is None or self.node is None:
            return
        msg = self.JointState()
        msg.header = self.Header(stamp=self.node.get_clock().now().to_msg(), frame_id=f"{self.hand_side}_hand")
        msg.name = JOINT_NAMES
        msg.position = np.asarray(qpos, dtype=float).reshape(-1).tolist()
...
def run_teleop(
    ...
    Args:
        ...
        hand_sn: Wuji Hand serial number (required when multiple hand devices are connected)
        ros_joint_state_topic: Optional ROS JointState topic for publishing retargeted commands
    """
    hand_side = hand_side.lower()
    assert hand_side in {"right", "left"}, "hand_side must be 'right' or 'left'"

    # Initialize hardware
    hand = wujihandpy.Hand(serial_number=hand_sn or None)
    ...
    ros_publisher = RosJointStatePublisher(ros_joint_state_topic, hand_side)
...
Examples:
  ...
  python teleop_real.py --play data/avp1.pkl --hand right --tuning
    ...
    parser.add_argument('--hand-sn', type=str, default='',
                        help='Wuji Hand serial number (required when multiple hand devices are connected)')
    parser.add_argument('--ros-joint-state-topic', type=str, default='',
                        help='Optional ROS JointState topic for retargeted joint commands')
